...
什么是解构赋值，举个例子。
ES6 允许按照一定模式，从数组和对象中提取值，对变量进行赋值，这被称为解构   let [a, b, c] = [1, 2, 3];    let { bar, foo } = { foo: 'aaa', bar: 'bbb' };
var {x, y = 5} = {x: 1}; x //1   y //5
默认值生效的条件是，对象的属性值严格等于undefined。  var {x = 3} = {x: undefined};     x   //3

装饰器的原理？

对于boss面问到的问题，我总结了一下其实这两个问题都很大。首先第一个问题，你在做什么？其实潜台词就是问你都做了什么项目，此时正确的做法是介绍自己参与的项目、
所在项目的角色，然后详细讲解你参与项目的技术方案，开发过程中遇到的问题以及如何去解决这些问题，
将主动权完全交给你，你可以自由发挥把面试官带到你的思路中。其实面试官更关心的是你解决问题的过程，
而不是像我一样两句话搞定。第二个问题就是要详细介绍自己的产品，产品和其他竞品的优势在哪里？
以及你贡献的内容和目前取得了什么样的成就等等。这种开放性的问题说好答也好答，说不好答也不好答，
完全看自己的语言组织能力。
...
浏览器对并发请求的数目限制是针对域名的，即针对同一域名（包括二级域名）在同一时间支持的并发请求数量的限制。如果请求数目超出限制，则会阻塞。
因此，网站中对一些静态资源，使用不同的一级域名，可以提升浏览器并行请求的数目，加速界面资源的获取速度。

浏览器与服务器建立一个TCP连接后，是否会在完成一个http请求后断开？什么条件下会断开？
在HTTP/1.0中，一个http请求收到服务器响应后，会断开对应的TCP连接。这样每次请求，都需要重新建立TCP连接，这样一直重复建立和断开的过程，比较耗时。
所以为了充分利用TCP连接，可以设置头字段Connection: keep-alive，这样http请求完成后，就不会断开当前的TCP连接，后续的http请求可以使用当前TCP连接进行通信。

一个TCP连接可以同时发送几个HTTP请求？
HTTP/1.1中，单个TCP连接，在同一时间只能处理一个http请求，虽然存在Pipelining技术支持多个请求同时发送，但由于实践中存在很多问题无法解决，所以浏览器默认是关闭，
所以可以认为是不支持同时多个请求。HTTP2提供了多路传输功能，多个http请求，可以同时在同一个TCP连接中进行传输。
...
你的项目中某个页面白屏了，请你分析一下白屏的原因(网络层，浏览器层面，业务逻辑层面)，并给出针对性的解决方案

类似于vuex这样的数据管理你会怎么去做？

treeSharking   有些代码会引起副作用，需要在代码里注意什么？
产生副作用的原因是babel编译，babel为了符合ES6真正的语义，编译类时采取了Object.defineProperty来定义原型方法，于是导致了后续这些一系列问题。
比如类内部声明的方法，是不可枚举的，而通过原型链声明的方法是可以枚举的，所以用Object.defineProperty()
比如使用了原型链、给window加了属性、立即执行函数引用了外部变量等一些你觉得没有副作用的经过bable其它的打包一下，也有可能会产生副作用
当下阶段，treeShaking上尽力能够做的事情：
